...
func (b *Botanist) DeployManagedResources(ctx context.Context) error {
	...
	b.BootstrapTokenSecret = bootstrapTokenSecret
	...
	var (
		...
		charts = map[string]managedResourceOptions{
			"shoot-cloud-config-execution": {false, b.generateCloudConfigExecutionChartForRBAC},
	...
	for _, worker := range b.Shoot.Info.Spec.Provider.Workers {
		releaseName := fmt.Sprintf("shoot-worker-cloud-config-%s", worker.Name)
		charts[releaseName] = managedResourceOptions{
			keepObjects:     false,
			chartRenderFunc: b.generateWorkerCloudConfigChartFunc(worker.Name),
		}
	}
